...
export default async function AdminPanelPage({
  ...
  const [deletedWords, deletedDefs, expired] = await Promise.all([
    ...
    prisma.opred_v.findMany({
      where: { is_deleted: true, word_v: { is_deleted: false } },
      orderBy: { id: "desc" },
      take: 200,
      select: {
        id: true,
  ...
  async function restoreDef(formData: FormData) {
    ...
    await prisma.opred_v.update({ where: { id: defId }, data: { is_deleted: false } });
    revalidatePath("/admin");
  }
  ...
  async function softDeleteDef(formData: FormData) {
    ...
    if (!id) return;
    const defId = BigInt(String(id));
    await prisma.opred_v.update({ where: { id: defId }, data: { is_deleted: true } });
    revalidatePath("/admin");
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-[220px_1fr] gap-6">
        <aside className="md:sticky md:top-4 h-max">
          <nav className="flex md:flex-col gap-2">
            <Button asChild variant={activeTab === "expired" ? "default" : "outline"}>
              <Link href={{ query: { tab: "expired" } }}>{t("expired")}</Link>
            </Button>
            <Button asChild variant={activeTab === "trash" ? "default" : "outline"}>
              <Link href={{ query: { tab: "trash" } }}>{t("deleted")}</Link>
            </Button>
          </nav>
        </aside>
        <main className="space-y-6">
          {activeTab === "expired" && (
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t("expired")} — {t("definitions")}</CardTitle>
              </CardHeader>
              <CardContent>
                {expired.length === 0 ? (
                  ...
                  <ul className="divide-y">
                    {expired.map((d) => (
                      <li key={String(d.id)} className="flex items-start justify-between gap-3 py-2">
                        <div className="flex-1 min-w-0">
                          <div className="text-sm text-rose-700 mb-1">
                            {t("word")}: {d.word_v?.word_text}
                          </div>
                          <div className="text-xs text-muted-foreground mb-1">
                            {t("expiresAt", { value: f.dateTime(d.end_date!, { dateStyle: "short" }) })}
                          </div>
                          <div className="break-words">{d.text_opr}</div>
                        </div>
                        <div className="flex flex-col items-end gap-2 shrink-0">
                          <form action={extendDef} className="flex items-center gap-2">
          ...
          {activeTab === "trash" && (
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t("deleted")} — {t("words")}</CardTitle>
              </CardHeader>
              <CardContent>
                {deletedWords.length === 0 ? (
                  ...
                  <ul className="divide-y">
                    {deletedWords.map((w) => (
                      <li key={String(w.id)} className="flex items-center gap-3 py-2">
                        <div className="flex-1 break-words">{w.word_text}</div>
                        <ServerActionButton
                          id={String(w.id)}
                          action={restoreWord}
                          labelKey="restore"
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t("deleted")} — {t("definitions")}</CardTitle>
              </CardHeader>
              <CardContent>
                {deletedDefs.length === 0 ? (
                  ...
                  <ul className="divide-y">
                    {deletedDefs.map((d) => (
                      <li key={String(d.id)} className="flex items-start gap-3 py-2">
                        <div className="flex-1 min-w-0">
