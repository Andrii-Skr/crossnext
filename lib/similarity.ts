...
export type Options = {
  topK?: number;
  duplicateThreshold?: number; // percent 0..100
  nearThreshold?: number; // percent 0..100
...
function normalizeText(raw: string, lang?: Lang, stop: ReturnType<typeof buildStopwords> = buildStopwords()) {
  ...
  // remove stopwords by language only
  ...
  // stem only for RU
  if (lang === "ru") {
    tokens = tokens.map((t) => natural.PorterStemmerRu.stem(t));
  }
  return tokens.join(" ");
}
...
export function prepareExisting(existing: ExistingDef[], options?: Options): PreparedExisting {
  ...
  return existing.map((e) => ({
    id: e.id,
    text: e.text,
    norm: normalizeText(e.text, e.lang, stop),
  }));
}
...
export function compareWithPrepared(newDef: NewDef, prepared: PreparedExisting, options?: Options): CompareResult {
  ...
  const similar = scored.filter((s) => s.percent >= nearThr);
  ...
  return {
    ...
    duplicates,
    similar,
    stats: { comparedCount: scored.length, elapsedMs },
  };
}
