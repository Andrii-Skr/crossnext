...
describe("/api/dictionary/word/[id] (PUT)", () => {
  ...
  it("updates word text and returns string id", async () => {
    prisma.word_v.update.mockResolvedValueOnce({ id: BigInt(123), word_text: "abc" });
    const req = makeReq("PUT", "http://localhost/api/dictionary/word/123", { word_text: "abc" });
    const res = await PUT(req as any, makeCtx({ id: "123" }));
  ...
  it("returns 404 on Prisma P2025 (not found)", async () => {
    ...
    const res = await PUT(req as any, makeCtx({ id: "1" }));
